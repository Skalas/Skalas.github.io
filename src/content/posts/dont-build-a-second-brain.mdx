...
The rulebook held, so I went after the other half. Every agent still ran its own copy of the dream code, and copies drift. I pulled them into one engine — [Mictlán](https://github.com/Skalas/mictlan) — with a thin adapter per agent. The adapters just gather what each agent learned; the engine does the consolidating, reading the same shared policy before anything lands in the vault.
